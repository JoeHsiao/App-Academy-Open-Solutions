...
const makeTree = (categories, parent) => {
  ...
  for (let i = categories.length-1; i >= 0; i--) {
    ...
    if (item.parent === parent) {
      directChildInd.push(i);
    }
  }
  
  let directChildObj = {}
  directChildInd.forEach(i => {
    obj = categories.splice(i, 1);
    directChildObj[obj[0].id] = {}
  })
  
  for (const nextParent in directChildObj) {
    subTree = makeTree(categories, nextParent);
    for (let k in subTree) {
      directChildObj[nextParent][k] = subTree[k]
    }
  }
  return directChildObj;
};
